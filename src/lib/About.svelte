...
<body id="abt">
    ...
    <main>
        <div class="doin">
            <div class="w2"><div class="idea" >
                <div bind:this={one}><p>In order to find my true passion than</p></div>
                <div bind:this={two}>
                    <p>spending my time playing football,</p>
                     
                </div>
                <div bind:this={three}><p>I have been actively exploring various fields.</p></div>
                <div bind:this={four}  use:viewport on:entry={entry} on:exit={exit}>
                    <p>
                        Currently, I am <span class="self">self-learning</span>
                        full-stack development using
                        <span class="open"> open-source resources</span>
                    </p>
                </div>
                <div bind:this={five}>to broaden my horizons.</div>
            </div></div>
...
<style>
    ...
    .idea>div:nth-child(1) {
        transform: translateY(2em);
        opacity: 0;
        transition: all 1.7s;
    }
    .idea>div:nth-child(2) {
        transform: translateY(2em);
        opacity: 0;
        transition: all 2s;
    }
    .idea>div:nth-child(3) {
        transform: translateY(2em);
        opacity: 0;
        transition: all 3s;
    }
    .idea>div:nth-child(4) {
        transform: translateY(2em);
        opacity: 0;
        transition: all 4s;
    }
    .idea>div:nth-child(5) {
        transform: translateY(2em);
        opacity: 0;
        transition: all 5s;
    }
